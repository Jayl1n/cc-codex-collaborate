...
# Generate curation index files if missing
for idx_file in source-index source-map curation-state; do
  target="docs/cccc/${idx_file}.json"
  template="$TEMPLATE_DIR/${idx_file}.template.json"
  if [[ ! -f "$target" ]]; then
    if [[ -f "$template" ]]; then
      cp "$template" "$target"
    else
      echo "{\"version\":1}" > "$target"
    fi
  fi
done

# Generate product docs if missing
for pf in product-notes business-notes monetization-notes go-to-market-notes; do
  target="docs/cccc/product/${pf}.md"
  if [[ ! -f "$target" ]]; then
    mkdir -p docs/cccc/product
    cat > "$target" <<EOF2
# ${pf}
