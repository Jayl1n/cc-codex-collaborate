...
- **`loop-start`** — Run `.claude/skills/cc-codex-collaborate/scripts/cccc-loop-start.sh` and act on CCCC_WORKFLOW_ACTION markers.
...
## loop-start behavior after enabling hooks

After running cccc-loop-start.sh, check the CCCC_WORKFLOW_ACTION marker:

- **`continue_now`** — Immediately continue the cc-codex-collaborate state machine. Do NOT stop after enabling hooks. Read config.json and state.json, then execute the next safe step.
- **`needs_resume`** — The workflow is paused. Suggest or execute `/cc-codex-collaborate resume`.
